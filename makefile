.SILENT: 
COMP=g++ -c -pipe -g -std=gnu++11 -Wall -W -D_REENTRANT -fPIC -DQT_DEPRECATED_WARNINGS -DQT_QML_DEBUG -DQT_WIDGETS_LIB -DQT_GUI_LIB -DQT_CORE_LIB -I../UNIX_Exercice1 -I. -isystem /usr/include/qt5 -isystem /usr/include/qt5/QtWidgets -isystem /usr/include/qt5/QtGui -isystem /usr/include/qt5/QtCore -I. -I. -I/usr/lib64/qt5/mkspecs/linux-g++ -o

CLINUX_Exercice1: main.o mainwindow.o moc_mainwindow.o
	echo Creation compilation de CLINUX_EXERCICE1
	g++  -o CLINUX_Exercice1 main.o mainwindow.o moc_mainwindow.o   /usr/lib64/libQt5Widgets.so /usr/lib64/libQt5Gui.so /usr/lib64/libQt5Core.so /usr/lib64/libGL.so -lpthread

main.o: main.cpp
	echo Creation compilation de main.o
	$(COMP)	main.o main.cpp

mainwindow.o: mainwindow.cpp mainwindow.h
	echo Creation compilation de mainwindow.o
	$(COMP) mainwindow.o mainwindow.cpp


moc_mainwindow.o: moc_mainwindow.cpp
	echo Creation compilation de moc_mainwindow.o
	$(COMP) moc_mainwindow.o moc_mainwindow.cpp

clean:
	rm -f *.o

clobber: clean
	rm -f CLINUX_Exercice1